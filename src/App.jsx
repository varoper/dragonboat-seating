import React, { useState, useEffect, useRef } from 'react';
import SeatingChart from './SeatingChart';
import Instructions from './Instructions';
import About from './About';
import { loadConfig } from './config/loadConfig';

function App() {
  const [currentTab, setCurrentTab] = useState('chart');
  const [config, setConfig] = useState({});

  const tabs = [
    { key: 'chart', label: 'Balancer' },
    { key: 'instructions', label: 'Instructions' },
    { key: 'about', label: 'About' },
  ];

  // Load configuration
  useEffect(() => {
    loadConfig().then(setConfig);
  }, []);

  const teamName = config.teamName || '';
  console.log("teamName", config.teamName);

  // Load correct tab
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const tab = params.get('tab');

    // Check if the tab key exists in the tabs array
    const isValidTab = tabs.some(({ key }) => key === tab);
    if (isValidTab) {
      setCurrentTab(tab);
    }
  }, []);

  return (
    <div className="App">
      <div className="px-3 md:px-6 lg:px-12 pt-3 border-b-2 border-slate-300 bg-gradient-to-b from-sky-50 to-sky-200">
        <h1>DragonBoat<span className="text-purple-600 font-bold">Seating</span> {teamName}</h1>
        <div className="inline-flex rounded-t-md overflow-hidden -mb-0.5">
          {tabs.map(({ key, label }) => (
            <a
              key={key}
              href={`?tab=${key}`}
              className={`px-2 sm:px-4 py-2 mr-2 text-md font-medium border border-slate-300 border-b-0 rounded-t-md transition-colors duration-150 ${currentTab === key
                ? 'bg-white text-purple-600'
                : 'bg-sky-50 border-b-2 text-slate-600 hover:text-purple-600'
                }`}
              onClick={(e) => {
                e.preventDefault();
                setCurrentTab(key);
                const url = new URL(window.location);
                url.searchParams.set('tab', key);
                window.history.pushState({}, '', url);
              }}
            >
              {label}
            </a>
          ))}
        </div>
      </div>

      <div className="bg-white py-6 px-3 md:px-6 lg:px-12 border-b-2 border-slate-300">
        <div className="max-w-6xl">
          {currentTab === 'chart' && (
            <SeatingChart />
          )}
          {
            currentTab === 'instructions' && (
              <Instructions />
            )
          }
          {
            currentTab === 'about' && (
              <About />
            )
          }
        </div>
      </div>
      <div className="py-6 px-3 md:px-6 lg:px-12">
        <p className="text-sm align-right">Created by <a target="_blank" href="https://varoper.com/">VA Roper</a> to make coaching just a little bit easier.</p>
      </div>
    </div >
  );
}

export default App;